...
app.get('/api/tracks', (req, res) => {
  const tracks = getTracksFromDisk();
  res.json(tracks);
});
...
app.delete('/api/admin/tracks/:id', requireAdminAuth, (req, res) => {
  const { id } = req.params;
  const tracks = getTracksFromDisk();
  ...
  saveTracksToDisk(updatedTracks);
...
app.post('/api/admin/reset', requireAdminAuth, (req, res) => {
  saveTracksToDisk(INITIAL_DEFAULT_TRACKS);
...
app.get('/api/content', (req, res) => {
  const content = getContentFromDisk();
  res.json(content);
});

app.put('/api/admin/content', requireAdminAuth, (req, res) => {
  try {
    const current = getContentFromDisk();
